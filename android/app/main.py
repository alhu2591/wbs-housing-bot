"""WBS Berlin v8.0 — Ultra-Modern UI · Live Theme · Full Customization"""
...
for _p in ["/sdcard/wbs_log.txt",
    ...
    try:
        with open(_p,"w") as _f:
            _f.write(f"WBS Berlin v8.0 — {time.ctime()}\nPython {sys.version}\n")
        _LOG = _p; break
...
def _init_arabic():
    ...
    try:
        ...
        log("arabic ok")
    except Exception as e: log(f"arabic: {e}")
...
def get_rows(limit=300):
    try:
        with _DL:
            ...
            try:
                rows = c.execute(
                    "SELECT * FROM listings WHERE hidden=0 ORDER BY ts_found DESC LIMIT ?",
                    (limit,)).fetchall()
...
def hide_row(lid):
    try:
        with _DL:
            c = _db()
            try:
                c.execute("UPDATE listings SET hidden=1 WHERE id=?",(lid,)); c.commit()
            finally: c.close()
    ...
    try:
        with _DL:
            ...
            try:
                ...
                new7  = c.execute("SELECT COUNT(*) FROM listings WHERE ts_found>?",
                                  (time.time()-7*86400,)).fetchone()[0]
                by_src= {row[0]:row[1] for row in
                         c.execute("SELECT source,COUNT(*) FROM listings GROUP BY source")}
                return {
                    "total": r[1] if r else 0, "opens": r[2] if r else 0,
                    "last_check": r[3] if r else None,
                    "db": total, "favs": favs, "new7": new7, "by_src": by_src
                }
            finally: c.close()
    except Exception: return {}
...
def purge_old(days=60):
    ...
    try:
        with _DL:
            c = _db()
            try:
                c.execute("DELETE FROM listings WHERE ts_found<? AND favorited=0",(cut,))
                c.commit()
            finally: c.close()
    except Exception: pass
...
# ══════════════════════════════════════════════════════════════════════
THEMES = {
    "غامق":   {"bg":"#080B14","bg2":"#0F1623","bg3":"#16202E","bg4":"#1E2D40",
                "tx1":"#E8F0FE","tx2":"#8FA3BF","tx3":"#4A6080","div":"#1E2D40"},
    "رمادي":  {"bg":"#0D0D0D","bg2":"#161616","bg3":"#202020","bg4":"#2A2A2A",
                "tx1":"#F5F5F5","tx2":"#A0A0A0","tx3":"#606060","div":"#2A2A2A"},
    "بحري":   {"bg":"#060D1F","bg2":"#0A1628","bg3":"#112040","bg4":"#1A305A",
                "tx1":"#DCE8FF","tx2":"#7A9CC4","tx3":"#3D6080","div":"#1A305A"},
    "زمردي":  {"bg":"#060E0A","bg2":"#0A1810","bg3":"#0F2218","bg4":"#163020",
                "tx1":"#D4F0E0","tx2":"#68A882","tx3":"#2E6050","div":"#163020"},
    "بنفسجي": {"bg":"#0C080F","bg2":"#160C1E","bg3":"#1E1030","bg4":"#281440",
                "tx1":"#EDE0FF","tx2":"#9A80C0","tx3":"#5A4080","div":"#281440"},
    "فاتح":   {"bg":"#F0F4FF","bg2":"#FFFFFF","bg3":"#E8EDF5","bg4":"#D8E0EE",
                "tx1":"#0D1B2A","tx2":"#4A6080","tx3":"#8FA3BF","div":"#D0D8E8"},
}
ACCENTS = {
    "أخضر نيون":  "#00FF87","أزرق برقي":  "#00B4FF","بنفسجي":     "#A855F7",
    "وردي":       "#FF2D78","برتقالي":     "#FF6B00","سماوي":      "#00D4D4",
    "ذهبي":       "#FFD700","أخضر":        "#22C55E","أزرق":       "#3B82F6",
    "أحمر":       "#EF4444",
}
...
SORT_OPTIONS = [
    ...
    ("rooms","🛏","أكثر غرفاً"),("size","📐","أكبر مساحة"),
]
...
DEF = {
    ...
    "purge_days":60,"compact_cards":False,"show_score":True,
}
...
FEATS = {
    ...
    "gasheizung":"🔥 غاز","saniert":"🔨 مجدد","waschmaschine":"🫧 غسالة",
    "badewanne":"🛁 حوض","sep. wc":"🚽 منفصل","rolladen":"🪟 ستائر",
    "penthouse":"🏙 بنتهاوس",
}
MONTHS_AR = {
...
_UA  = "Mozilla/5.0 (Linux; Android 14; SM-A536B) AppleWebKit/537.36 Chrome/124.0"
...
def enrich(title, desc):
    ...
    if m:
        ...
        m=re.search(r"(\d)\s*monatsmieten?",t)
...
if HAS_KIVY:
    # ── Live theme state ─────────────────────────────────────────────
    _TH   = dict(THEMES["غامق"])
    _AC   = [0.0, 1.0, 0.53, 1.0]   # neon green default
    _FS   = [1.0]
    _FN   = ["Roboto"]

    def _load_theme(cfg):
        th_name = cfg.get("theme","غامق")
        acc_name = cfg.get("accent","أخضر نيون")
        _TH.update(THEMES.get(th_name, THEMES["غامق"]))
        c = get_color_from_hex(ACCENTS.get(acc_name,"#00FF87"))
        _AC[:] = list(c)
        _FS[0] = FONT_SIZES.get(cfg.get("font_size","عادي"),1.0)
    ...
    _PUR = get_color_from_hex("#A855F7")
    _BLU = get_color_from_hex("#3B82F6")
    _AMB = get_color_from_hex("#F59E0B")
    _RED = get_color_from_hex("#EF4444")
    _GLD = get_color_from_hex("#FFD700")
    _GRN = get_color_from_hex("#22C55E")
    WH   = (1,1,1,1)
    NO   = (0,0,0,0)
    ...
    def outline(w, col, r=0, width=1.2):
        """Add a thin border outline."""
        with w.canvas.after:
            Color(*col)
            Line(rounded_rectangle=(w.x,w.y,w.width,w.height,dp(r)),
                 width=width)
        def u(*_):
            w.canvas.after.clear()
            with w.canvas.after:
                Color(*col)
                Line(rounded_rectangle=(w.x,w.y,w.width,w.height,dp(r)),width=width)
        w.bind(pos=u,size=u)
    ...
    def bt(text, cb=None, col=None, tc=None, h=48, r=14, outlined=False, **kw):
        ...
        bg(b, col, r=r)
        if outlined: outline(b, (*col[:3],0.5), r=r)
    ...
    def ghost_bt(text, cb=None, col=None, h=40, r=12, **kw):
        ...
        b = Button(text=txt, size_hint_y=None, height=dp(h),
                   background_color=NO, color=col,
                   font_size=fs(13), bold=True, font_name=FN(), **kw)
        bg(b, (*col[:3],0.08), r=r)
        outline(b, (*col[:3],0.5), r=r)
        if cb: b.bind(on_press=cb)
        return b
    ...
    def chip_btn(text, active=False, col=None, on_toggle=None, h=30, r=20, **kw):
        ...
        b = ToggleButton(text=ar(str(text)), state="down" if active else "normal",
                         ...
                         font_size=fs(11), bold=active, font_name=FN(),
                         size_hint_y=None, height=dp(h), **kw)
        def upd(inst, state):
            on=state=="down"
            bg(inst, (*col[:3],0.82) if on else BG3(), r=r)
            inst.color = TX1() if on else TX2()
            inst.bold  = on
            if on: outline(inst, (*col[:3],0.0), r=r)
    ...
    def inp(val, filt="int", hint="", **kw):
        t = TextInput(text=str(val), input_filter=filt, multiline=False,
                      ...
                      cursor_color=AC(), hint_text_color=TX3(),
                      font_size=fs(14), hint_text=hint, **kw)
        bg(t, BG3(), r=12)
        outline(t, (*AC()[:3],0.3), r=12)
        return t
    ...
    def section_hdr(text, col=None):
        row = BoxLayout(size_hint_y=None, height=dp(40), padding=(dp(2),dp(4)))
        row.add_widget(lb(text, sz=11, col=col or TX3(), bold=True))
        return row

    def pill_label(text, col, bg_alpha=0.15, sz=10, h=22, w_hint=None):
        ww = w_hint or dp(len(str(text))*7+16)
        b = BoxLayout(size_hint=(None,None), size=(ww,dp(h)), padding=(dp(6),0))
        bg(b, (*col[:3],bg_alpha), r=h//2)
    ...
    def navbar(app_ref, active):
        ...
        with bar.canvas.after:
            Color(*(*AC()[:3],0.3))
            Line(points=[0,bar.height,bar.width,bar.height], width=1)
        for icon,name,label in TABS:
            ...
            b = Button(text=txt, background_color=NO,
                       ...
                       font_size=fs(9 if not on else 10),
    ...
    class ListingCard(BoxLayout):
        def __init__(self, l, compact=False, **kw):
            ...
            src_col   = _PUR if gov else _BLU
            price     = l.get("price");   rooms = l.get("rooms")
            sz        = l.get("size_m2"); avail = l.get("available","")
            floor_    = l.get("floor_") or l.get("floor","")
            ...
            self.height = dp((128 if compact else 162) + n_fr*22) + extra_h
            bg(self, BG2(), r=18)
            if sc_>=20: outline(self, (*AC()[:3],0.35), r=18)

            # ── Row 1: badges ────────────────────────────────────────
            r1 = BoxLayout(size_hint_y=None, height=dp(26), spacing=dp(6))
            r1.add_widget(pill_label(("🏛 " if gov else "🔍 ")+ar(name), src_col, h=24))
            if sc_>=22: r1.add_widget(pill_label("⭐⭐⭐", _GLD, h=24, w_hint=dp(52)))
            elif sc_>=16: r1.add_widget(pill_label("⭐⭐", _GLD, h=24, w_hint=dp(44)))
            elif sc_>=10: r1.add_widget(pill_label("⭐", _GLD, h=24, w_hint=dp(34)))
            r1.add_widget(Widget())
            if wlbl: r1.add_widget(pill_label(wlbl, AC(), bg_alpha=0.20, sz=10, h=24, w_hint=dp(74)))
            self._fb = Button(text="★" if is_fav else "☆",
                              size_hint=(None,None), size=(dp(30),dp(24)),
                              background_color=NO, color=_GLD if is_fav else TX3(),
                              font_size=sp(17))
            ...
            if not compact or avail:
                ...
                if avail:
                    ...
                    r2.add_widget(lb(ar(avail_txt), sz=10,
                                     col=_AMB if avail=="فوري" else TX2()))
                self.add_widget(r2)
            ...
            for icon,val in [("🛏",f"{rooms:.0f}" if rooms else None),
                ...
                if val:
                    ...
                    bg(db, BG3(), r=9)
                    db.add_widget(lb(f"{icon} {val}".strip(), sz=10, col=TX1(),
            ...
            ab.add_widget(ghost_bt("إخفاء", cb=self._hide, col=TX3(),
                                    h=36, r=10, size_hint_x=None, width=dp(78)))
            self.add_widget(ab)

        def _fav(self,*_):
            if not self.lid: return
            nv = toggle_fav(self.lid)
            self._fb.color = _GLD if nv else TX3()
            self._fb.text  = "★" if nv else "☆"

        def _hide(self,*_):
            if self.lid: hide_row(self.lid)
            anim = Animation(opacity=0, height=0, duration=0.22, t="out_quad")
            anim.start(self)
    ...
    class MainScreen(Screen):
        ...
        def _ui(self):
            ...
            bg(bar,BG2())
            # Logo + title
            title_row=BoxLayout(orientation="vertical",size_hint_x=0.45)
            title_row.add_widget(lb("WBS Berlin",sz=15,bold=True,col=AC()))
            title_row.add_widget(lb("شقق برلين المدعومة",sz=9,col=TX3()))
            bar.add_widget(title_row); bar.add_widget(Widget())
            # Controls
            sort_icons={k:i for k,i,_ in SORT_OPTIONS}
            self._sort_bt=icon_bt(sort_icons.get(cfg.get("sort_by","score"),"🏅"),
                                   cb=self._cycle_sort,size=40)
            bar.add_widget(self._sort_bt)
            self._cmp_bt=icon_bt("▦" if self._compact else "▤",
                                   cb=self._toggle_compact,size=40)
            bar.add_widget(self._cmp_bt)
            self._rf=bt("🔄",cb=self._go,col=AC(),
                         size_hint_x=None,h=40,width=dp(46),r=12)
            bar.add_widget(self._rf)
            root.add_widget(bar)

            # ── Filter strip ──────────────────────────────────────────
            fs_=BoxLayout(size_hint_y=None,height=dp(46),
                           padding=(dp(10),dp(7)),spacing=dp(8))
            bg(fs_,BG2())
            self._wbs_c=chip_btn(ar("✅ WBS"),active=bool(cfg.get("wbs_only")),
                                  col=AC(),on_toggle=self._wbs_toggled,
                                  size_hint_x=None,width=dp(88))
            fs_.add_widget(self._wbs_c)
            self._stat_lb=lb("اضغط 🔄 للبحث",sz=11,col=TX2(),
                               size_hint_y=None,height=dp(28))
            fs_.add_widget(self._stat_lb)
            fs_.add_widget(Widget())
            self._bg_dot=Label(text="⏸",font_size=sp(18),color=TX3(),
                                size_hint=(None,None),size=(dp(28),dp(28)))
            fs_.add_widget(self._bg_dot)
            root.add_widget(fs_)
            root.add_widget(hdiv())

            # ── Cards list ────────────────────────────────────────────
            self._cl=BoxLayout(orientation="vertical",spacing=dp(10),
            ...
            root.add_widget(navbar(self.app,"main"))
            self.add_widget(root)
            self._placeholder("🔍",ar("اضغط 🔄 للبحث"))
        ...
        def _cycle_sort(self,*_):
            keys=[k for k,_,_ in SORT_OPTIONS]
            icons={k:i for k,i,_ in SORT_OPTIONS}
            cfg=load_cfg(); cur=cfg.get("sort_by","score")
            nxt=keys[(keys.index(cur)+1)%len(keys)]
            cfg["sort_by"]=nxt; save_cfg(cfg)
            self._sort_bt.text=icons[nxt]
            with self._lock: raw=list(self._raw)
            if raw:
        ...
        def _placeholder(self,icon,msg):
            ...
            b=BoxLayout(orientation="vertical",spacing=dp(14),size_hint_y=None,
                        height=dp(210),padding=dp(44))
        ...
        def _go(self,*_):
            ...
            if not check_net():
                ...
                else: self._placeholder("📵",ar("لا يوجد اتصال"))
                return
    ...
    class FavsScreen(Screen):
        ...
        def _build(self):
            ...
            bar=BoxLayout(size_hint_y=None,height=dp(60),padding=(dp(14),dp(10)),spacing=dp(10))
            bg(bar,BG2())
            bar.add_widget(lb("⭐ "+ar("المفضلة"),sz=16,bold=True,col=_GLD))
            bar.add_widget(Widget())
            bar.add_widget(icon_bt("🔄",cb=self._load,size=40))
            root.add_widget(bar)
            self._cl=BoxLayout(orientation="vertical",spacing=dp(10),
                                padding=(dp(10),dp(10)),size_hint_y=None)
            self._cl.bind(minimum_height=self._cl.setter("height"))
            sv=ScrollView(bar_color=(*_GLD[:3],0.5),bar_inactive_color=(0,0,0,0))
            sv.add_widget(self._cl); root.add_widget(sv)
            root.add_widget(navbar(self.app,"favs")); self.add_widget(root)
            self._load()
        ...
        def _load(self,*_):
            ...
            if not favs:
                ...
                b.add_widget(lb("اضغط ★ على أي إعلان لحفظه",sz=12,col=TX3(),size_hint_y=None,height=dp(36)))
                self._cl.add_widget(b); return
            for l in favs:
                self._cl.add_widget(ListingCard(l)); self._cl.add_widget(gp(6))
    ...
    class StatsScreen(Screen):
        ...
        def _build(self):
            ...
            root=BoxLayout(orientation="vertical")
            bar=BoxLayout(size_hint_y=None,height=dp(60),padding=(dp(14),dp(10)))
            bg(bar,BG2())
            bar.add_widget(lb("📊 "+ar("الإحصائيات"),sz=16,bold=True,col=WH))
            root.add_widget(bar)
            sc=ScrollView(); body=BoxLayout(orientation="vertical",
                padding=dp(14),spacing=dp(10),size_hint_y=None)
            ...
            # Summary cards row
            sum_row=BoxLayout(size_hint_y=None,height=dp(90),spacing=dp(8))
            def mini_card(icon,val,lbl_t,col):
                c=BoxLayout(orientation="vertical",padding=(dp(12),dp(10)),spacing=dp(4))
                bg(c,BG2(),r=16)
                ...
                return c
            sum_row.add_widget(mini_card("🏠",st.get("db",0),"محفوظ",AC()))
            sum_row.add_widget(mini_card("🆕",st.get("new7",0),"هذا الأسبوع",_GRN))
            sum_row.add_widget(mini_card("⭐",st.get("favs",0),"مفضلة",_GLD))
            sum_row.add_widget(mini_card("📱",st.get("opens",0),"فتحات",_PUR))
            body.add_widget(sum_row)

            # Last check
            lc_row=BoxLayout(size_hint_y=None,height=dp(48),padding=(dp(14),dp(8)))
            bg(lc_row,BG2(),r=14)
            lc_row.add_widget(lb("🕐 "+ar("آخر فحص:"),sz=12,col=TX2(),size_hint_x=0.45))
            lc_row.add_widget(lb(ts(st.get("last_check")),sz=12,col=TX1(),align="left"))
            body.add_widget(lc_row)

            # Per-source breakdown
            body.add_widget(gp(4)); body.add_widget(section_hdr("📡  المصادر"))
            by_src=st.get("by_src",{}); max_cnt=max(by_src.values()) if by_src else 1
            for src,(name,gov) in SOURCES.items():
                cnt=by_src.get(src,0)
                row=BoxLayout(size_hint_y=None,height=dp(46),padding=(dp(12),dp(6)),spacing=dp(8))
                bg(row,BG2(),r=12)
                sc_=_PUR if gov else _BLU
                row.add_widget(lb(("🏛 " if gov else "🔍 ")+ar(name),sz=12,col=sc_,size_hint_x=0.5))
                # mini progress bar
                prog=BoxLayout(size_hint=(0.35,None),height=dp(8))
                bg(prog,BG3(),r=4)
                fw=max(0.04,cnt/max(max_cnt,1))
                fill=Widget(size_hint=(fw,1)); bg(fill,sc_,r=4)
                rest_=Widget(size_hint=(1-fw,1))
                prog.add_widget(fill); prog.add_widget(rest_)
                row.add_widget(prog)
                row.add_widget(lb(str(cnt),sz=13,bold=True,col=AC(),
                                   size_hint_x=0.15,align="left"))
                body.add_widget(row)
    ...
    # ══════════════════════════════════════════════════════════════════
    # SETTINGS — with live theme rebuild
    # ══════════════════════════════════════════════════════════════════
    class CfgScreen(Screen):
        ...
        def _build(self):
            ...
            root=BoxLayout(orientation="vertical")
            hdr=BoxLayout(size_hint_y=None,height=dp(60),padding=(dp(14),dp(10)),spacing=dp(10))
            bg(hdr,BG2())
            hdr.add_widget(lb("⚙️ "+ar("الإعدادات"),sz=16,bold=True,col=WH))
            hdr.add_widget(Widget())
            hdr.add_widget(ghost_bt("↩️ "+ar("افتراضي"),cb=self._reset,col=TX2(),
                                     h=36,r=12,size_hint_x=None,width=dp(100)))
            root.add_widget(hdr)
            ...
            def row(lbl_t,w,hint=""):
                r=BoxLayout(size_hint_y=None,height=dp(58),spacing=dp(12),padding=(dp(14),dp(7)))
                bg(r,BG2(),r=14)
            ...
            # ── 🎨 المظهر ─────────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("🎨  المظهر"))
            th_row=BoxLayout(size_hint_y=None,height=dp(52),padding=(dp(14),dp(8)),spacing=dp(8))
            bg(th_row,BG2(),r=14); th_row.add_widget(lb("نسق:",sz=12,col=TX1(),size_hint_x=0.2))
            self._th_btns={}; cur_th=cfg.get("theme","غامق")
            for th_name in THEMES:
                on=th_name==cur_th
                b=ToggleButton(text=ar(th_name),state="down" if on else "normal",
                    size_hint=(1,None),height=dp(36),background_color=NO,
                    color=TX1() if on else TX2(),font_size=fs(10),bold=on)
                bg(b,(*AC()[:3],0.22) if on else BG3(),r=10)
                b.bind(state=lambda x,s,n=th_name,b=b:(
                    bg(b,(*AC()[:3],0.22) if s=="down" else BG3(),r=10),
                    setattr(b,"color",TX1() if s=="down" else TX2()),
                    self._desel_theme(n) if s=="down" else None))
                self._th_btns[th_name]=b; th_row.add_widget(b)
            body.add_widget(th_row)

            # Accent
            acc_row=BoxLayout(size_hint_y=None,height=dp(60),padding=(dp(14),dp(10)),spacing=dp(5))
            bg(acc_row,BG2(),r=14); acc_row.add_widget(lb("لون:",sz=12,col=TX1(),size_hint_x=0.14))
            self._acc_btns={}; cur_acc=cfg.get("accent","أخضر نيون")
            for name,hex_ in ACCENTS.items():
                col=get_color_from_hex(hex_)
                on=name==cur_acc
                b=Button(text="",size_hint=(None,None),size=(dp(32),dp(32)),
                         background_color=NO)
                bg(b,col,r=16)
                if on: outline(b,WH,r=16,width=2)
                b.bind(on_press=lambda _,n=name:self._sel_acc(n))
                self._acc_btns[name]=(b,col); acc_row.add_widget(b)
            body.add_widget(acc_row)

            # Font size
            fs_row=BoxLayout(size_hint_y=None,height=dp(52),padding=(dp(14),dp(8)),spacing=dp(8))
            bg(fs_row,BG2(),r=14); fs_row.add_widget(lb("خط:",sz=12,col=TX1(),size_hint_x=0.18))
            self._fs_btns={}; cur_fs=cfg.get("font_size","عادي")
            for sz_name in FONT_SIZES:
                ...
                b=ToggleButton(text=ar(sz_name),state="down" if on else "normal",
                    ...
                    color=TX1() if on else TX2(),font_size=fs(10),bold=on)
                bg(b,(*AC()[:3],0.22) if on else BG3(),r=10)
                b.bind(state=lambda x,s,n=sz_name,b=b:(
                    bg(b,(*AC()[:3],0.22) if s=="down" else BG3(),r=10),
                    self._desel_fs(n) if s=="down" else None))
                self._fs_btns[sz_name]=b; fs_row.add_widget(b)
            body.add_widget(fs_row)

            # Compact cards
            self._compact=tog("🗜 وضع البطاقات المضغوطة",cfg.get("compact_cards",False))

            # ── 💰 الميزانية ───────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("💰  الميزانية"))
            self._maxp=inp(cfg.get("max_price",700),hint="أقصى سعر"); row("أقصى إيجار (€)",self._maxp)
            self._minp=inp(cfg.get("min_price",0),hint="0=بدون"); row("الحد الأدنى (€)",self._minp,"0=بدون حد")

            # Rooms + Size
            body.add_widget(gp(4)); body.add_widget(section_hdr("🛏  الغرف والمساحة"))
            def range_row(lbl_t, w1, w2, unit):
                rr=BoxLayout(size_hint_y=None,height=dp(58),spacing=dp(6),padding=(dp(14),dp(7)))
                bg(rr,BG2(),r=14)
                rr.add_widget(lb(lbl_t,sz=12,col=TX1(),size_hint_x=0.28))
                rr.add_widget(lb("من",sz=10,col=TX2(),size_hint_x=0.07)); rr.add_widget(w1)
                rr.add_widget(lb("—",sz=12,col=TX2(),size_hint_x=0.05)); rr.add_widget(w2)
                rr.add_widget(lb(unit,sz=10,col=TX3(),size_hint_x=0.12))
                body.add_widget(rr)
            self._minr=inp(cfg.get("min_rooms",0),"float"); self._maxr=inp(cfg.get("max_rooms",0),"float")
            range_row("الغرف",self._minr,self._maxr,"0=أي")
            self._mins=inp(cfg.get("min_size",0)); self._maxs=inp(cfg.get("max_size",0))
            range_row("المساحة",self._mins,self._maxs,"m²")

            # ── 📋 WBS ──────────────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("📋  WBS"))
            self._wbs=tog("WBS فقط",cfg.get("wbs_only",False))
            wlr=BoxLayout(size_hint_y=None,height=dp(58),spacing=dp(6),padding=(dp(14),dp(7)))
            bg(wlr,BG2(),r=14)
            ...
            wlr.add_widget(lb("من",sz=10,col=TX2(),size_hint_x=0.07)); wlr.add_widget(self._wlmin)
            wlr.add_widget(lb("—",sz=12,col=TX2(),size_hint_x=0.05)); wlr.add_widget(self._wlmax)
            body.add_widget(wlr)
            ...
            # ── 🏛 اجتماعي ─────────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("🏛  الفلاتر الاجتماعية"))
            ...
            # ── 📍 المناطق ──────────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("📍  المناطق"))
            ...
            for area in AREAS:
                ...
                b=ToggleButton(text=area,state="down" if on else "normal",
                    size_hint=(1,None),height=dp(36),background_color=NO,color=TX1(),font_size=fs(11))
            ...
            body.add_widget(ag)
            body.add_widget(ghost_bt("🌍 "+ar("كل برلين"),
                cb=lambda *_:[setattr(b,"state","normal") or bg(b,BG2(),r=10) for b in self._ab.values()],
                col=TX2(),h=38,r=12))

            # ── 🌐 المصادر ──────────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("🌐  مصادر البيانات"))
            ...
            qr.add_widget(ghost_bt("✅ "+ar("الكل"),
                cb=lambda *_:[setattr(b,"state","down") for b in self._src.values()],col=AC(),h=40,r=12))
            qr.add_widget(ghost_bt("🏛 "+ar("حكومية فقط"),cb=self._govsrc,col=_PUR,h=40,r=12))
            body.add_widget(qr)

            # ── ⚙️ متقدم ────────────────────────────────────────────────
            body.add_widget(gp(4)); body.add_widget(section_hdr("⚙️  إعدادات متقدمة"))
            self._bgi=inp(cfg.get("bg_interval",30)); row("فترة الخلفية (دق.)",self._bgi,"5+")
            self._pdays=inp(cfg.get("purge_days",60)); row("حذف تلقائي (يوم)",self._pdays,"60=افتراضي")
            bg_on=is_bg()
        ...
        def _desel_theme(self,chosen):
            for n,b in self._th_btns.items():
                if n!=chosen:
                    b.state="normal"; bg(b,BG3(),r=10); b.color=TX2(); b.bold=False

        def _desel_fs(self,chosen):
            for n,b in self._fs_btns.items():
                if n!=chosen:
                    b.state="normal"; bg(b,BG3(),r=10); b.color=TX2(); b.bold=False

        def _sel_acc(self,name):
            for n,(b,col) in self._acc_btns.items():
                b.canvas.after.clear()
                if n==name: outline(b,WH,r=16,width=2)
        ...
        def _save(self,*_):
            ...
            cur_th =next((n for n,b in self._th_btns.items() if b.state=="down"),"غامق")
            cur_acc=next((n for n,(b,c) in self._acc_btns.items() if b.canvas.after.children),"أخضر نيون")
            ...
            cfg.update({
                ...
                "min_size":      si(self._mins,0),
                "max_size":      si(self._maxs,0),
                ...
                "purge_days":    max(7,si(self._pdays,60)),
            ...
            save_cfg(cfg)
            # Apply live theme — then rebuild ALL screens so colors refresh
            _load_theme(cfg)
            Window.clearcolor=BG()
            self.app.rebuild_all(cfg)
    ...
    class OnboardScreen(Screen):
        ...
        def _show(self):
            ...
            brow=BoxLayout(size_hint=(0.9,None),height=dp(56),
            ...
            brow.add_widget(bt("ابدأ الآن 🚀" if last else "التالي ←",
                                cb=self._next if not last else self._done,col=p[3],h=52,r=16))
    ...
    class SplashScreen(Screen):
        def __init__(self,**kw):
            ...
            root=FloatLayout()
            card=BoxLayout(orientation="vertical",padding=dp(32),spacing=dp(16),
                           size_hint=(0.78,0.50),pos_hint={"center_x":.5,"center_y":.5})
            ...
            card.add_widget(lb("v8.0",sz=12,col=TX3(),size_hint_y=None,height=dp(24)))
    ...
    class WBSApp(App):
        ...
        def rebuild_all(self,cfg):
            """Rebuild all screens after theme change — fixes stale colors."""
            log("rebuild_all")
            try:
                cur=self.sm.current
                # Remove all screens except splash
                to_remove=[s for s in list(self.sm.screens) if s.name!="splash"]
                for s in to_remove: self.sm.remove_widget(s)
                # Re-add fresh
                for name,cls in [("main",MainScreen),("favs",FavsScreen),
                                  ("stats",StatsScreen),("cfg",CfgScreen)]:
                    self.sm.add_widget(cls(self))
                # Navigate back to main (settings saved → go home)
                self.sm.current="main"
            except Exception as e:
                log(f"rebuild_all err:{e}")
