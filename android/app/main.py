...
# ── Crash log (before everything) ────────────────────────────────────
_T0 = time.time()
_LOG = None
for _p in ["/sdcard/wbs_log.txt", os.path.join(os.path.dirname(os.path.abspath(__file__)), "wbs_log.txt")]:
    try:
        open(_p, "w").write(f"START {time.ctime()}\nPython {sys.version}\n")
        _LOG = _p
        break
    except Exception:
        pass
...
def _find_dir():
    if os.path.exists("/data/data"):
        for d in ["/sdcard/WBSBerlin", "/sdcard"]:
            try:
                os.makedirs(d, exist_ok=True)
                open(os.path.join(d, ".t"), "w").write("ok")
                os.unlink(os.path.join(d, ".t"))
                return d
            except Exception:
                pass
    for d in [os.path.expanduser("~/.wbs"), "."]:
        try:
            os.makedirs(d, exist_ok=True)
            return d
        except Exception:
...
FEATS = {
    ...
    "stellplatz":"🚗 موقف", "barrierefrei":"♿",
    "neubau":"🏗 جديد", "fußbodenheizung":"🌡 تدفئة", "saniert":"🔨 مجدد",
}
...
def enrich(title, desc):
    ...
    if any(k in t for k in ["ab sofort", "sofort frei"]):
        out["available"] = "فوري"
    else:
...
def scrape_gewobag():
    ...
    for i in items:
        ...
        l = {
            "id": make_id(url), "url": url, "source": "gewobag", "trusted_wbs": True,
...
def scrape_degewo():
    for api in [
        ...
        for i in items:
            ...
            l = {
                "id": make_id(url), "url": url, "source": "degewo", "trusted_wbs": True,
...
def apply_filters(listings, cfg):
    ...
    for l in listings:
        if not l.get("id") or l.get("hidden"):
            continue
        if srcs and l.get("source") not in srcs:
            continue
        price = l.get("price")
        rooms = l.get("rooms")
        if price is not None:
            ...
            if price > max_p:
                continue
        if rooms is not None and min_r > 0 and rooms < min_r:
            continue
        if wbs and not l.get("trusted_wbs"):
            continue
        level = l.get("wbs_level") or l.get("wbs_level_num")
        if level is not None and (wlmin > 0 or wlmax < 999):
            if not (wlmin <= level <= wlmax):
                continue
        if areas:
...
def start_bg():
    ...
    if _bg_th and _bg_th.is_alive():
        return
    _bg_stop.clear()
    _bg_th = threading.Thread(target=_bgw, daemon=True)
    _bg_th.start()
    log("bg started")
...
def is_bg():
    return bool(_bg_th and _bg_th.is_alive())
...
if HAS_KIVY:
    ...
    def si(t, d=0):
        try:
            return int(float(t.text or d))
        except Exception:
            return d

    def sf(t, d=0.0):
        try:
            return float(t.text or d)
        except Exception:
            return d
    ...
    class Card(BoxLayout):
        def __init__(self, l, **kw):
            ...
            try:
                feats = json.loads(l["features"]) if isinstance(l.get("features"), str) else (l.get("features") or [])
            except Exception:
                feats = []
            feats = feats[:4]
            ...
            ch.add_widget(lb(("🏛 " if gov else "🔍 ") + ar(name), sz=10, col=sc, size_hint_y=None, height=dp(20)))
            r1.add_widget(ch)
            r1.add_widget(Widget())
            if wlbl:
                wb = BoxLayout(size_hint=(None, None), size=(dp(68), dp(20)), padding=(dp(5), 0))
                bg(wb, (*AC()[:3], 0.18), r=10)
                wb.add_widget(lb(wlbl, sz=10, col=AC(), bold=True, size_hint_y=None, height=dp(20)))
                r1.add_widget(wb)
            ...
            self._fb.bind(on_press=self._fav)
            r1.add_widget(self._fb)
            self.add_widget(r1)
            ...
            if avail:
                ...
                r3.add_widget(lb("📅 " + ar(avail_txt), sz=10, col=AMB if avail == "فوري" else TX2))
            self.add_widget(r3)
            self.add_widget(dv())
            ...
            if price:
                ...
                bg(p2, (*AC()[:3], 0.15), r=8)
                p2.add_widget(lb(f"💰 {price:.0f}€{ppm}", sz=11, col=AC(), bold=True, size_hint_y=None, height=dp(26)))
                r4.add_widget(p2)
            if rooms:
                r4.add_widget(lb(f"🛏 {rooms:.0f}", sz=11, col=TX1))
            if sz:
                r4.add_widget(lb(f"📐 {sz:.0f}m²", sz=11, col=TX1))
            self.add_widget(r4)
    ...
    class MainScreen(Screen):
        ...
        def _ui(self):
            ...
            bar = BoxLayout(size_hint_y=None, height=dp(54), padding=(dp(12), dp(8)), spacing=dp(8))
            bg(bar, BG2)
            ...
            chips = BoxLayout(size_hint_y=None, height=dp(40), padding=(dp(10), dp(5)), spacing=dp(8))
            bg(chips, BG2)
            ...
            sv = ScrollView(bar_color=(*AC()[:3], 0.4))
            sv.add_widget(self._cl)
            root.add_widget(sv)
        ...
        def _render(self, lst, total=None, nc=0):
            try:
                ...
                if not lst:
                    ...
                    self._ph("🔍", ar("لا توجد إعلانات"))
                    return
                ns = f" (+{nc})" if nc else ""
                self._st.text = ar(f"✅ {len(lst)} من {t}{ns}")
                for l in lst[:10]:
                    self._cl.add_widget(Card(l))
    ...
    class FavsScreen(Screen):
        ...
        def _build(self):
            ...
            bar.add_widget(bt("🔄", cb=self._load, col=BG3, tc=TX2, size_hint_x=None, h=38, width=dp(42)))
            root.add_widget(bar)
            self._cl = BoxLayout(orientation="vertical", spacing=dp(8),
                                  padding=(dp(10), dp(8)), size_hint_y=None)
            self._cl.bind(minimum_height=self._cl.setter("height"))
            sv = ScrollView(bar_color=(*AMB[:3], 0.4))
            sv.add_widget(self._cl)
            root.add_widget(sv)
            root.add_widget(navbar(self.app, "favs"))
            self.add_widget(root)
            self._load()
        ...
        def _load(self, *_):
            ...
            favs = get_favs()
            if not favs:
                b = BoxLayout(orientation="vertical", size_hint_y=None, height=dp(140), padding=dp(28))
    ...
    class CfgScreen(Screen):
        ...
        def _build(self):
            ...
            cfg = load_cfg()
            root = BoxLayout(orientation="vertical")
            hdr = BoxLayout(size_hint_y=None, height=dp(54), padding=(dp(12), dp(8)), spacing=dp(10))
            bg(hdr, BG2)
            hdr.add_widget(lb("⚙️ " + ar("الإعدادات"), sz=15, bold=True, col=WH))
            hdr.add_widget(bt("↩️", cb=self._reset, col=BG3, tc=TX2, size_hint_x=None, h=38, width=dp(46)))
            root.add_widget(hdr)

            sc = ScrollView()
            body = BoxLayout(orientation="vertical", padding=dp(14), spacing=dp(8), size_hint_y=None)
            body.bind(minimum_height=body.setter("height"))

            def row(lbl_t, w, hint=""):
                r = BoxLayout(size_hint_y=None, height=dp(52), spacing=dp(12), padding=(dp(12), dp(4)))
                bg(r, BG2, r=12)
                lb2 = BoxLayout(orientation="vertical", size_hint_x=0.45)
                lb2.add_widget(lb(lbl_t, sz=12, col=TX1))
                if hint:
                    lb2.add_widget(lb(hint, sz=10, col=TX3))
                r.add_widget(lb2)
                r.add_widget(w)
                body.add_widget(r)
            ...
            def tog(text, active, pri=None):
                ...
                t = ToggleButton(text=ar(text), state="down" if active else "normal",
                                  ...
                                  background_color=NO, color=TX1, font_size=fs(12), font_name=FN())
                bg(t, (*pri[:3], 0.15) if active else BG2, r=12)
                t.bind(state=lambda b, s, p=pri: bg(b, (*p[:3], 0.15) if s == "down" else BG2, r=12))
                body.add_widget(t)
                return t

            self._maxp = inp(cfg.get("max_price", 700))
            row("أقصى إيجار (€)", self._maxp)
            self._minp = inp(cfg.get("min_price", 0))
            row("الحد الأدنى (€)", self._minp, "0=بدون")
            self._minr = inp(cfg.get("min_rooms", 0), "float")
            row("أقل غرف", self._minr, "0=أي")
            ...
            body.add_widget(gp(6))
            body.add_widget(self._bgb)
            body.add_widget(gp(8))
            body.add_widget(bt("💾 " + ar("حفظ"), cb=self._save, h=50, r=14))
            body.add_widget(gp(20))

            sc.add_widget(body)
            root.add_widget(sc)
            root.add_widget(navbar(self.app, "cfg"))
            self.add_widget(root)
        ...
        def _togbg(self, *_):
            if is_bg():
                stop_bg()
                bg(self._bgb, AC(), r=12)
                self._bgb.text = ar("▶ تشغيل الخلفية")
            else:
                start_bg()
                bg(self._bgb, RED, r=12)
    ...
    class SplashScreen(Screen):
        def __init__(self, **kw):
            ...
            card.add_widget(lb("WBS Berlin", sz=19, bold=True, col=GRN, size_hint_y=None, height=dp(38)))
            card.add_widget(lb("جاري التحميل...", sz=13, col=TX2, size_hint_y=None, height=dp(28)))
    ...
    class OnboardScreen(Screen):
        ...
        def _show(self):
            ...
            if not last:
                brow.add_widget(bt("تخطي", cb=self._done, col=BG3, tc=TX2))
    ...
    if __name__ == "__main__":
        ...
        init_db()
        raw = []
        for fn in [scrape_gewobag, scrape_degewo]:
            try:
                raw.extend(fn())
            except Exception:
                pass
