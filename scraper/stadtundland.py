"""scraper/stadtundland.py — Stadt und Land scraper."""
...
SOURCE = "stadtundland"
BASE_URL = "https://www.stadtundland.de/Expose/Immobilien.htm"
...
async def scrape(cfg: dict[str, Any]) -> list[dict]:
    ...
    soup = make_soup(html)
    listings = []
    for card in soup.select(".expose-item, .immobilien-item, article, .result-item"):
        try:
            title_el = card.select_one("h3, h2, .expose-title")
            ...
            logger.debug("stadtundland error: %s", e)
    logger.info("stadtundland: %d listings", len(listings))
    return listings
